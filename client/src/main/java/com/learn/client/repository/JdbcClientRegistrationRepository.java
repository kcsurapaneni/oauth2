...
public class JdbcClientRegistrationRepository implements ClientRegistrationRepository, Iterable<ClientRegistration> {
    ...
    public ClientRegistration findByRegistrationId(String registrationId) {
        return jdbcTemplate.query(FETCH_BY_ID, this::mapToClient, registrationId).get(0);
    }

    private ClientRegistration mapToClient(ResultSet rs, int rowNum) throws SQLException {
        return ClientRegistrations.fromIssuerLocation(rs.getString("issuer_uri"))
                .registrationId(rs.getString("id"))
